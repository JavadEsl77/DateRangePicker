...
export const DateRangePicker = () => {
  ...
  const displayStart = dateRange.start ? formatDateToInput(dateRange.start, calendarType) : (calendarType === 'persian' ? 'شروع' : 'Start');
  const displayEnd = dateRange.end ? formatDateToInput(dateRange.end, calendarType) : (calendarType === 'persian' ? 'پایان' : 'End');

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-8 flex items-center justify-center">
      <Card className="w-full max-w-5xl p-8 shadow-xl bg-white">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            />
            <Label htmlFor="calendar-type" className="text-sm font-medium cursor-pointer">
        ...
        <div className="mb-6">
          <p className="text-xs font-semibold text-primary tracking-wider mb-2">
            {calendarType === 'persian' ? 'انتخاب بازه تاریخ' : 'SELECT DATE RANGE'}
          </p>
